...
async function loadUnitData(unitNumber) {
    try {
        const response = await fedocument.addEventListener('DOMContentLoaded', () => {
    const urlParams = new URLSearchParams(window.location.search);
    const unitNumber = urlParams.get('unit');
    ...
    async function loadUnitData(unitNumber) {
        try {
            // Nota: Se hai rinominato il file JSON in "data.json", assicurati che il percorso sia 'data/data.json'
    ...
    function renderUnitContent(unitData) {
        ...
        unitData.sections.forEach((section, index) => { // 
            ...
            // Il player audio è ora globale, quindi non lo aggiungiamo qui
            // Aggiungiamo un link al titolo della sezione per poterla riprodurre
